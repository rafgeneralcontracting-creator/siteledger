(function(){
function canEditSubmitted(){return !!me}
...
async function patch(){
  ...
  page.querySelectorAll('input,textarea,select').forEach(el=>{
    if(!el.getAttribute('onchange')&&el.closest('.field')){
      ...
      const map={'General Conditions':'general_conditions','Work Performed':'work_performed','Areas / Floors':'areas_floors','Delays / Impacts':'delays_impacts','Visitors / Inspections':'visitors_inspections','Safety Notes':'safety_notes',"Tomorrow's Plan":'tomorrow_plan'};
  ...
  if(mp&&!mp.querySelector('.sl-owner-add-manpower')){const b=document.createElement('button');b.className='btn secondary block sl-owner-add-manpower';b.textContent='+ Add Manpower';b.onclick=()=>manpowerForm();mp.appendChild(b)}
  const ph=sectionCard('Progress Photos');
  if(ph&&!ph.querySelector('.sl-owner-add-photo')){const b=document.createElement('button');b.className='btn secondary block sl-owner-add-photo';b.style.marginTop='12px';b.textContent='+ Add Progress Photos';b.onclick=()=>photoForm();ph.appendChild(b)}
  const di=sectionCard('Deliveries & Issues');
  if(di&&!di.querySelector('.sl-owner-add-delivery')){const b1=document.createElement('button');b1.className='btn secondary block sl-owner-add-delivery';b1.textContent='+ Add Delivery';b1.onclick=()=>deliveryForm();di.appendChild(b1);const b2=document.createElement('button');b2.className='btn secondary block sl-owner-add-issue';b2.textContent='+ Report Issue';b2.onclick=()=>issueForm();di.appendChild(b2)}
}
