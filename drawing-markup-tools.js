(function(){
  ...
  let sheet=null, marks=[], active=false, tool='pen', color='#dc2626', stroke=null, loadingKey='', saving=false;
  ...
  function render(){
    ...
    marks.forEach(m=>drawStroke(ctx,m,c.width,c.height)); if(stroke)drawStroke(ctx,stroke,c.width,c.height);
  ...
  window.setDrawingMarkupTool=function(t){tool=t;stroke=null;toolbar();render()};
  ...
  window.finishDrawingMarkup=function(){active=false;stroke=null;q('sl_markup_session_bar')?.remove();document.body.classList.remove('sl-drawing-markup-active');render();document.getElementById('sl_field_pan')?.click()};
