...
(() => {
  ...
  window.slDownloadWeekly=async(path,name)=>{try{const url=await weeklySigned(path),res=await fetch(url);if(!res.ok)throw new Error('Could not download Weekly PDF.');const blob=await res.blob(),href=URL.createObjectURL(blob),a=document.createElement('a');a.href=href;a.download=name||'Weekly-Report.pdf';document.body.appendChild(a);a.click();a.remove();setTimeout(()=>URL.revokeObjectURL(href),1000)}catch(e){alert(e.message)}};
  window.slShareWeekly=async(path,name)=>{try{const url=await weeklySigned(path),res=await fetch(url);if(!res.ok)throw new Error('Could not load Weekly PDF.');const blob=await res.blob(),file=new File([blob],name||'Weekly-Report.pdf',{type:'application/pdf'});if(navigator.share&&navigator.canShare?.({files:[file]}))await navigator.share({title:'Weekly Report',files:[file]});else{const href=URL.createObjectURL(blob),a=document.createElement('a');a.href=href;a.download=name||'Weekly-Report.pdf';document.body.appendChild(a);a.click();a.remove();setTimeout(()=>URL.revokeObjectURL(href),1000);alert('The Weekly PDF was downloaded. Attach it to your email from your mail app.')}}catch(e){alert(e.message)}};
